...
class PreprocessRawBytesTest(tf.test.TestCase):
    def setUp(self):
        super(PreprocessRawBytesTest, self).setUp()
        ...
        # length of each context value in bytes
        self._factor_encoder = 4 if self._config_encoder['output_dtype'] == tf.int32 else 1
    ...
    def test_specs(self):
        ...
        self.assertEqual(__contexts_spec.shape, (self._config_encoder['batch_dim'], self._config_encoder['sample_dim'] // (self._factor_encoder * self._config_encoder['token_dim']), self._config_encoder['token_dim']))
...
class PreprocessBinaryBytesTest(tf.test.TestCase):
    ...
    def test_inputs(self):
        ...
        for _ in range(2):
            ...
            __y = tf.cast(__y, tf.float32) / tf.cast(self._config_decoder['input_dim'], tf.float32)
