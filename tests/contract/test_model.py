...
class BinaryTransformerTest(tf.test.TestCase):
    def setUp(self):
        ...
        self._config_encoder = {'batch_dim': 4, 'sample_dim': 66 * 4 * 128, 'input_dim': 66 * 4, 'sequence_axis': 1, 'feature_axis': -1,}
        ...
        self._config_model = {'num_layers': 4, 'num_heads': 8, 'input_dim': 33 * 4, 'context_dim': 66 * 4, 'embed_dim': 1056, 'head_dim': 132, 'hidden_dim': 4224,}
        ...
        self._dataset_before = tf.data.Dataset.from_tensor_slices({
            ...
            'creation_sourcecode': 8 * [1056 * b'a']})
        ...
        # build
        __x = tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_decoder['input_dim']], dtype=tf.int32)
        __c = tf.zeros([self._config_encoder['batch_dim'], (self._config_encoder['sample_dim'] // self._config_encoder['input_dim']), self._config_encoder['input_dim']], dtype=tf.int32)
        self._model((__x, __c))

    def test_internals(self):
        # embeddings
        assert list(self._model._embed_input._embeddings.shape) == [256, self._config_model['embed_dim'] // self._config_model['input_dim']]
        assert list(self._model._embed_context._embeddings.shape) == [256, self._config_model['embed_dim'] // self._config_model['context_dim']]
        # blocks
        ...
        assert all(list(__b._cross_attention._attention._key_dense.kernel.shape) == [self._config_model['embed_dim'], self._config_model['num_heads'], self._config_model['head_dim']] for __b in self._model._blocks)
        ...
        assert all(list(__b._cross_attention._attention._value_dense.kernel.shape) == [self._config_model['embed_dim'], self._config_model['num_heads'], self._config_model['head_dim']] for __b in self._model._blocks)
        # ffn
        ...
        # head
        assert list(self._model._project.kernel.shape) == [self._config_model['embed_dim'], 8 * self._config_model['input_dim']]
        # assert list(self._model._project.bias.shape) == [8 * self._config_model['input_dim']]
    ...
    def test_null_values(self):
        ...
        # cross attention
        __x = tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32)
        __c = tf.zeros([self._config_encoder['batch_dim'], self._config_encoder['sample_dim'] // self._config_encoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32)
        self.assertAllEqual(self._model._blocks[0]._cross_attention(inputs=__x, contexts=__c), tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32))
        # ffn
        __x = tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32)
        self.assertAllEqual(self._model._blocks[0]._ffn(__x), tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32))
        # head
        __x = tf.zeros([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], self._config_model['embed_dim']], dtype=tf.float32)
        self.assertAllEqual(self._model._project(__x), 0.5 * tf.ones([self._config_decoder['batch_dim'], self._config_decoder['sample_dim'] // self._config_decoder['input_dim'], 8 * self._config_model['input_dim']], dtype=tf.float32))
